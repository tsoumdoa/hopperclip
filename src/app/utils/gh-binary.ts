...
class GhBinaryReader {
	...
	private readChunkWithDepth(maxDepth: number): GhChunk {
		...
		if (itemCount < 0 || chunkCount < 0) {
			throw new Error("Invalid Grasshopper binary chunk counts");
		}
...
class SizeBoundedWriter {
	...
	push(line: string): void {
		...
		if (this.length > this.maxLength) {
			throw new GhSizeError(
				"Grasshopper binary XML output exceeds size limit"
			);
		}
...
function itemToXml(item: GhItem, depth: number, writer: SizeBoundedWriter) {
	const indent = "  ".repeat(depth);
	const typeName = TYPE_NAMES[item.typeCode] ?? "gh_unknown";
	const index = item.index >= 0 ? ` index="${item.index}"` : "";
	writer.push(
		`${indent}<item name="${escapeXml(item.name)}"${index} type_name="${typeName}" type_code="${item.typeCode}">${itemValueToXml(item, indent)}</item>`
	);
}

function chunkToXml(chunk: GhChunk, depth: number, writer: SizeBoundedWriter) {
	const indent = "  ".repeat(depth);
	const index = chunk.index >= 0 ? ` index="${chunk.index}"` : "";
	writer.push(`${indent}<chunk name="${escapeXml(chunk.name)}"${index}>`);

	if (chunk.items.length > 0) {
		writer.push(`${indent}  <items count="${chunk.items.length}">`);
		for (const item of chunk.items) {
			itemToXml(item, depth + 2, writer);
		}
		writer.push(`${indent}  </items>`);
	}

	if (chunk.chunks.length > 0) {
		writer.push(`${indent}  <chunks count="${chunk.chunks.length}">`);
		for (const child of chunk.chunks) {
			chunkToXml(child, depth + 2, writer);
		}
		writer.push(`${indent}  </chunks>`);
	}

	writer.push(`${indent}</chunk>`);
}

function archiveToXml(root: GhChunk, maxOutputBytes: number) {
	const writer = new SizeBoundedWriter(maxOutputBytes);

	writer.push('<?xml version="1.0" encoding="utf-8" standalone="yes"?>');
	writer.push(`<Archive name="${escapeXml(root.name)}">`);
	writer.push("  <!--Grasshopper archive-->");
	writer.push("  <!--Grasshopper and GH_IO.dll are copyrighted by Robert McNeel & Associates-->");
	writer.push("  <!--Archive generated by GH_IO.dll file utility library {0.2.0002}-->");

	if (root.items.length > 0) {
		writer.push(`  <items count="${root.items.length}">`);
		for (const item of root.items) {
			itemToXml(item, 2, writer);
		}
		writer.push("  </items>");
	}

	if (root.chunks.length > 0) {
		writer.push(`  <chunks count="${root.chunks.length}">`);
		for (const chunk of root.chunks) {
			chunkToXml(chunk, 2, writer);
		}
		writer.push("  </chunks>");
	}
...
export function grasshopperBinaryToXml(
	bytes: Uint8Array,
	maxOutputBytes = MAX_DECOMPRESSED_GH_XML_BYTES
) {
	...
	return archiveToXml(root, maxOutputBytes);
}
